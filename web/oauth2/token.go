...
func handleTokenRequest(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, &TokenResponse{Error: "invalid_request", ErrorDescription: err.Error()})
		return
	}

	db := panelmiddleware.GetDatabase(c)
	if db == nil {
		c.JSON(http.StatusInternalServerError, &TokenResponse{Error: "invalid_request", ErrorDescription: "database not available"})
		return
	}
	...
	case "client_credentials":
		{
			...
			if !client.ValidateSecret(request.ClientSecret) {
				c.JSON(http.StatusBadRequest, &TokenResponse{Error: "invalid_client"})
				return
			}
	...
	case "password":
		{
			...
			if auth == "" || !strings.HasPrefix(auth, "Bearer ") {
				...
				c.JSON(http.StatusUnauthorized, &TokenResponse{Error: "invalid_client"})
				return
			}
			...
			if server.NodeID != node.ID {
				c.JSON(http.StatusBadRequest, &TokenResponse{Error: "invalid_request", ErrorDescription: "no access"})
				return
			}
	...
	default:
		c.JSON(http.StatusBadRequest, &TokenResponse{Error: "unsupported_grant_type"})
	}
}
