...
func (op SpongeDl) Run(env daemon.Environment) error {
	...
	case "spongeforge":
		{
			...
			forgeDlOp, err := forgedl.Factory.Create(daemon.CreateOperation{OperationArgs: mapping})
			if err != nil {
				return err
			}
...
func (op SpongeDl) getLatestVersion(env daemon.Environment) (SpongeApiV2Versions, error) {
...
func (op SpongeDl) getSpecificVersion(env daemon.Environment, version string) (SpongeApiV2Latest, error) {
