...
import (
	...
	"github.com/pufferpanel/pufferpanel/v3/daemon/operations"
...
type Program struct {
	...
	RunningEnvironment daemon.Environment `json:"-"`
...
func (p *Program) Start() (err error) {
	...
	err = p.RunningEnvironment.ExecuteAsync(daemon.ExecutionData{
		Command:          cmd,
		Arguments:        args,
...
func (p *Program) SetEnvironment(environment daemon.Environment) (err error) {
...
func (p *Program) GetEnvironment() daemon.Environment {
...
func (p *Program) afterExit(exitCode int) {
	...
	if exitCode == 0 && p.Execution.AutoRestartFromGraceful {
		StartViaService(p)
	} else if exitCode != 0 && p.Execution.AutoRestartFromCrash && p.CrashCounter < config.CrashLimit.Value() {
		p.CrashCounter++
		StartViaService(p)
	}
}
